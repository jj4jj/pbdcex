...
struct array_t {
    ...
    int lower_bound(const T & tk) const {
        T * p = std::lower_bound(list, list + count, tk);
        if (p == list + count){
            return -1;
        }
    ...
    int upper_bound(const T & tk) const {
        T * p = std::upper_bound(list, list + count, tk);
        if (p == list + count){
            return -1;
        }
        return (p - list) / sizeof(T);
...
template<class T, size_t cmax, class hcf, size_t factor=75>
struct hashmap_t {
