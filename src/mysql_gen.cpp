...
typedef std::unordered_map<std::string, ::google::protobuf::Message*>	MySQLMsgInstCache;
struct MySQLMsgMetaImpl {
	...
	MySQLMsgInstCache	msg_inst_cache;
};
...
MySQLMsgMeta::~MySQLMsgMeta() {
	if(impl){
		for(auto & pr: impl->msg_inst_cache){
			impl->protometa.FreeDynMessage(pr.second);
		}
		impl->msg_inst_cache.clear();
		impl->msg_meta_cache.clear();
		delete impl; 
		impl = nullptr;
	}
}
...
std::string		MySQLMsgMeta::GetMsgNameFromTableName(const std::string & tname) {
	int pos = tname.find_last_of(TABLE_NAME_IDX_SEP);
	if (pos != std::string::npos && tname[pos] >= '1' && tname[pos] <= '9') {
		return tname.substr(0, pos);
	}
	return tname;
}
...
int			MySQLMsgMeta::GetMsgBuffFromSQLRow(std::string & buff, const MySQLRow &  sql_row) {
	std::string msgname = GetMsgNameFromTableName(sql_row.table_name);
	auto it = impl->msg_inst_cache.find(msgname);
	::google::protobuf::Message * msg = nullptr;
	if (it == impl->msg_inst_cache.end()) {
		msg = it->second;
	}
	else {
		auto mp = impl->protometa.NewDynMessage(msgname.c_str());
		if(!mp){
			GLOG_ERR("create instance of msg type:%s fail!", msgname.c_str());
			return -1;
		}
		impl->msg_inst_cache[msgname] = mp;
		msg = mp;
	}
	int ret = GetMsgFromSQLRow(*msg, sql_row);
	if (ret) {
		GLOG_ERR("get msg from sql row error:%d table name:%s!", ret, sql_row.table_name);
		return -1;
	}
	if (!msg->SerializeToString(&buff)) {
		GLOG_ERR("parse from buffer msg error type name:%s", msgname.c_str());
		return -1;
	}
	return 0;
}
