...
static inline std::string cpp_get_scalar_type_name(EXTFieldMeta * em) {
	...
	else if (em->field_desc->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
		return em->field_desc->message_type()->name() + "_ST";
	}
	else if (em->field_desc->cpp_type() == FieldDescriptor::CPPTYPE_STRING) {
		std::string type_name;
		if (em->field_desc->type() == FieldDescriptor::TYPE_STRING) {
			type_name = "string_t<";
			type_name += em->length;
			type_name += ">";
		}
...
static inline string cpp_get_type_name(EXTFieldMeta * em)  {
	...
	if (em->vcount > 0) {
		type_name = "array_t<" + type_name + ", " + em->count + ">";
	}
	return type_name;
}
static inline string cpp_get_var_name(EXTFieldMeta * em)  {
	//static const char * type_prefix = ["", "i", "ll", "dw", "ull", "df", "f", "b", "en", "str", "st"];
	//desc->camelcase_name();
	string lc_name = em->field_desc->lowercase_name();
	return lc_name;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int     CXXFlatMsgGenerater::Init(const char * pszTmpl){
    ...
    xctmp_push_filter(xctmp, "field.is_enum", [](const string & p)->string {
        EXTFieldMeta * m = (EXTFieldMeta*)(strtoull(p.c_str(), NULL, 16));
        if(m->field_desc->cpp_type() == google::protobuf::FieldDescriptor::CPPTYPE_ENUM){
            return "1";
        }
        return "0";
    });
...
int	CXXFlatMsgGenerater::GetCodeText(::std::string & output,const char * tf){
    ...
    for (int i = 0; i < (int)vems.size(); ++i){
        ...
        jenv += "],\"pkfields\":[";
        head = true;
        for (auto & sfm : vems[i].keys_fields){
            if (!head){
                jenv += ",";
            }
            head = false;
        ...
        jenv += "],\"pkfields_num\":" + ::std::to_string(vems[i].keys_fields.size());
        jenv += "}";
    }
    jenv += "]}";
...
void	CXXFlatMsgGenerater::TopologySort(::std::vector<const Descriptor * > & msgs){
    ...
    while (!msg_degrees.empty()){
        ...
        if (it == msg_degrees.end()){
            GLOG_ERR("msg degree not found zero !");
        ...
        for (auto desc : reverse_refer[it->first]){
            if (msg_degrees[desc] > 0){
                ...
                GLOG_ERR("msg type:%s  refer count is 0 !",desc->name().c_str());
            }
        }
